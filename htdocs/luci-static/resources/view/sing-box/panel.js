...
const actions = { validate: '校验', apply: '应用', start: '启动', stop: '停止', restart: '重启' };
...
return view.extend({
	...
	render(data) {
		...
		this.dirty = E('p', { class: 'cbi-section-descr', 'aria-live': 'polite' });
		...
		const config = E('div', { 'data-tab': 'config', 'data-tab-title': '配置' }, [
			E('p', {}, '直接编辑 JSON。校验不保存；保存并应用会先写入文件，校验通过后重启服务。'),
			this.dirty,
			this.editor,
		...
		this.logHint = E('p', { role: 'status' });
		this.logUpdated = E('p', { class: 'cbi-section-descr' });
		...
		const logs = E('div', { 'data-tab': 'logs', 'data-tab-title': '日志' }, [
			this.logHint,
			this.logUpdated,
			this.logs,
		]);
		...
		const root = E('div', { class: 'cbi-map', id: 'sing-box-panel' }, [
			...
			E(
				'p',
				{ class: 'cbi-map-descr' },
				this.writable ? '配置与服务管理' : '当前账号仅有查看权限。',
			),
			this.problem,
			this.progress,
